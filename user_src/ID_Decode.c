...
/*  Mark        :ver 1.0                                               */
/***********************************************************************/
#include <iostm8l151g4.h> // CPUĺĺˇ
#include "Pin_define.h"   // çŽĄčĺŽäš
#include "initial.h"      // ĺĺ§ĺ? é˘ĺŽäš?
#include "ram.h"          // RAMĺŽäš
#include "ID_Decode.h"
...
//void EXIT_init(void)
//{
//    EXTI_CR1 = 0x20;          //PORT B2  çä¸­ć­č§Śĺä˝
//    ADF7021_DATA_CLK_CR2 = 1; //ä˝żč˝čŻĽI/OĺŁä¸­ć? PA1
//    //   EXTI_CR2=0X00;
//    //   PIN_PD7_CR2=1;
//}
/**
...
void ID_Decode_IDCheck(void)
{
    ...
    if (FLAG_Receiver_IDCheck)
    {
        ...
        if (FLAG_Signal_DATA_OK == 1)
        {
            ...
            {
                if ((FLAG_ID_Login_OK == 0) && (DATA_Packet_Contro_buf != 0x40) && (DATA_Packet_ID != 0)&&(Radio_Date_Type_bak==1)) //2015.4.1äżŽć­Ł ĺ¨çťĺ˝ć¨Ąĺźä¸ ä¸ĺčŽ¸čŞĺ¨é?äżĄçťĺ˝ďźĺŞĺčŽ¸ćĺ¨éäżĄçťĺ˝
                {
            ...
            else if ((FLAG_IDCheck_OK == 1) || (DATA_Packet_ID == 0xFFFFFE))
            {
			    ...
			    if(Radio_Date_Type_bak==1)
			    {
		                if (DATA_Packet_ID == 0xFFFFFE)
		                    DATA_Packet_Control = DATA_Packet_Contro_buf; //2015.3.24äżŽć­Ł Controlçźĺ­čľ?IDĺ¤ć­ćŻĺŚĺ­Śäš čżĺćč˝ä˝żç¨
		                ...
		                if ((SPI_Receive_DataForC[1] & 0x0000FFFF) == 0x5556)
		                {
		                    ...
		                    if (FLAG_Signal_DATA_OK == 1)
		                    {
		                        ...
		                        { //2015.4.2äżŽć­Ł  éäżĄĺ¨ćčžšççťĺ˝é?čż˝ĺ çťĺ˝ä¸čĄ
		                ...
		                else
		                {
		                    ...
		                    else
		                    {
		                        ...
		                        TIME_auto_close = 0;
		                        FG_auto_open_time = 0;
		                        if (FG_auto_manual_mode == 1)      //Manual_override_TIMER=13500;   //2ĺ?0ç§čŞĺ¨ć ć?
		                            Manual_override_TIMER = 24480; //4ĺ?0ç§čŞĺ¨ć ć?
	...
	if(FLAG_testNo91==1)
		...
		if(Radio_Date_Type_bak==1)
		{
                ...
                do
				{
					...
					} //2015.3.24010703030205 Control0404Ą°02020š0¨¨08?ID020ˇ3Ąč03¨C02030š004020607020207010102¨¨076Ś002060503Ąë03¨¨060501050704ĄąĄ§
                    ...
                    if ((FLAG_ID_Erase_Login == 1) && (FLAG_ID_Erase_Login_PCS == 1))
					{
						...
						FLAG_IDCheck_OK = 0;
						DATA_Packet_Control = DATA_Packet_Contro_buf;
					} //¨¨070502070202Ąč08030103ID046ž40302056Ś1
                    i++;
                } while (i < ID_DATA_PCS);
        }
...
/*
   time_beepONătime_beepOFFĺä˝ćśé´ä¸?0.4333333ms
*/
void BEEP_Module(UINT16 time_beepON, UINT16 time_beepOFF)
{
	...
	{
		//Receiver_Buzzer=!Receiver_Buzzer;   //čé¸Łĺ¨é˘ç?.08KHZ
		if (FG_beep_on == 0)
		{
			FG_beep_on = 1;
			FG_beep_off = 0;
	...
	{
		// Receiver_Buzzer=0;	//čé¸Łĺ¨é˘ç?.08KHZ
		if (FG_beep_off == 0)
		{
			FG_beep_off = 1;
			FG_beep_on = 0;
...
//				            	}						
		...
		if((FLAG__Semi_open_T==1)||(FLAG__Semi_close_T==1)){
					 if((DATA_Packet_Control==0x02)||(DATA_Packet_Control==0x04)||(DATA_Packet_Control==0x08)||(DATA_Packet_Control==0x01)||(DATA_Packet_Control==0x20)||(DATA_Packet_Control==0x40)
					  ||(DATA_Packet_Control==0x9)||(DATA_Packet_Control==0x03)||(DATA_Packet_Control==0x0C)||(DATA_Packet_Control==0x06)||(DATA_Packet_Control==0x0A)){
						 //2015.12.29čż˝ĺ ďźĺ¨ĺĺźăĺé­ĺ¨ä˝ä¸­ďźé?äżĄćşďźĺź?+é­ďźäżĄĺˇďźčŽŠĺć­˘çť§çľĺ¨ä¸ĺ¨ä˝
    ...
    else
    {
        ...
        //           if(FLAG_426MHz_Reply==1){FLAG_426MHz_Reply=0;ID_data.IDL=DATA_Packet_ID;Control_code=HA_Status+4;FLAG_HA_START=1;}   //ĺäżĄĺ¨čŞĺ¨ĺéHAçść?ç ä¸şĺŽéHAç ?4
        ...
        if ((FLAG_ID_Erase_Login == 1) || (FLAG_ID_Login == 1) || (TIME_auto_close)||(FLAG_ID_SCX1801_Login==1))
            ...
            Receiver_LED_OUT = 0;
               ...
               if((FLAG__Semi_open_T==1)||(FLAG__Semi_close_T==1)){
//                   if(HA_Status==0x83)TIMER250ms_STOP=0;     //2015.12.29čż˝ĺ ďźĺ¨ĺĺźăĺé­ĺ¨ä˝ä¸­ďźĺäżĄćşççśćĺćĺźĺ¸?1çćśĺďźčŽŠĺć­˘çť§çľĺ¨ä¸ĺ¨ä˝?
               ...
               else if((TIMER250ms_STOP==0)&&(TIME_auto_close==0)){Receiver_OUT_STOP=FG_NOT_allow_out;FG_OUT_OPEN_CLOSE=0;}    //2015.3.23äżŽćš		
...
/*
			if ((Flag_FREQ_Scan == 0)&&((FLAG_ID_Erase_Login == 1) || (FLAG_ID_Login == 1) ||(FLAG_ID_SCX1801_Login==1)||
			   ...
			   )  //ďż˝ďż˝ďż˝ďż˝ÄŁĘ˝Ęąďż˝ďż˝ďż˝ďż˝ďż˝ďż˝426.075MHzďż˝ďż˝ďż˝ĹşĹŁďż˝Öťďż˝ďż˝ďż˝Úľďż˝ÂźÄŁĘ˝Ęąďż˝Ĺ˝ďż˝ďż˝ÜĄďż˝
*/
			{
