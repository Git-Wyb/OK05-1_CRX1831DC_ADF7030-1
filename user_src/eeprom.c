...
/*  Mark        :STM8S207C8鐨凜ODE绌洪棿涓?4K                             */
/*              :STM8S207C8鐨凟EPROM鐨勫ぇ灏忎负1536瀛楄妭,鍗?3椤?512鑺?椤?   */
/***********************************************************************/
...
void ID_learn(void)
{
    ...
    { //90==1绉?
        ...
        if (Receiver_Login == 0)
        {
            ...
            {
                Register_state = ENTER_ADD_STATE;
                Flag_Register_tx = 1;
                FLAG_ID_Login = 1;
        ...
        if(TIME_Receiver_Login >= 450)  //590
        {
	      ...
	      {
                Register_state = ENTER_ERASE_STATE;
                Flag_Register_tx = 1;
	            //TIME_Receiver_Login = 0;
